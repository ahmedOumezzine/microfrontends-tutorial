...
export default function ChartList({ onReportSelected = () => {}, selectedReport = 'conversion' }) {
  ...
  useEffect(() => {
    const chart = new Chart(canvasRef.current, {
      type: 'bar',
      data: {
        labels: ['Build', 'Test', 'Audit', 'Deploy'],
        datasets: [{ label: 'Minutes CI', data: [4, 3, 2, 1], backgroundColor: ['#0d6efd', '#198754', '#ffc107', '#6c757d'] }],
      },
    ...
    <div className="card">
      ...
      <div className="card-body">
        ...
        <div className="btn-group mt-3">
          ...
          <button className={`btn btn-outline-dark ${selectedReport === 'quality' ? 'active' : ''}`} onClick={() => onReportSelected('quality')}>Qualite</button>
        </div>
      </div>
    </div>
  );
